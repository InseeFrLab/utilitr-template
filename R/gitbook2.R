...
bs4_book2 <- function(
  ...
  config <- rmarkdown::html_document(
    ...
    theme = NULL,
    template = bookdown:::bookdown_file("templates", "bs4_book.html"),
    pandoc_args = bookdown:::pandoc_args2(pandoc_args),
    lib_dir = lib_dir,
    extra_dependencies = c(bookdown:::bs4_book_dependency(theme),
  ...
  config$post_processor <- function(metadata, input, output, clean, verbose) {
    ...
    output2 <- bookdown:::bs4_book_build(output, repo = repo, lib_dir = lib_dir)

    if (clean && file.exists(output) && !xfun::same_path(output, output2)) {
      file.remove(output)
    }
    output2
  }
...
jquery_dependency <- bookdown:::jquery_dependency
...
gitbook_dependency = function(table_css) {
  ...
  list(jquery_dependency(), htmltools::htmlDependency(
    ...
    stylesheet = file.path('css', c(
      'style.css', if (table_css) 'plugin-table.css',
      'plugin-bookdown.css',
...
gitbook2 = function(
  ...
  config = rmarkdown::html_document(
    ...
    pandoc_args = bookdown:::pandoc_args2(pandoc_args),
    extra_dependencies = c(gitbook_dependency(table_css),
                           utilitr_dependencies(),
                           extra_dependencies),
    ...
  )
...
write_search_data = function() {
  ...
  x = matrix(bookdown:::strip_search_text(x), nrow = 3)
  x = apply(x, 2, xfun::json_vector, to_array = TRUE)
  x = paste0('[\n', paste0(x, collapse = ',\n'), '\n]')
  x = gsub('[^[:print:]]', '', x)
  xfun::write_utf8(x, bookdown:::output_path('search_index.json'))
}
gitbook_page = function(
  ...
  # from head to foot
  # utilitr-HACK
  i = grep('^\\s*<script src=".+/(gitbook|utilitr)([^/]+)?/js/[.a-z-]+[.]js"></script>\\s*$', head)
  ...
  if (is.null(bookdown:::opts$get('book_filename'))) {
    config$search = FALSE
  } else {
  ...
  for (type in c('edit', 'history', 'view')) {
    if (is.list(setting <- bookdown:::source_link_setting(config[[type]], type = type)))
      config[[type]] = setting
...
gitbook_toc = function(x, cur, config) {
  i1 = bookdown:::find_token(x, '<!--bookdown:toc2:start-->')
  i2 = bookdown:::find_token(x, '<!--bookdown:toc2:end-->')
  x[i1] = ''; x[i2] = ''
  if (i2 - i1 < 2) return(x)
  toc = x[(i1 + 1):(i2 - 1)]
  # numbered sections
  ...
  toc[i] = sub(' data-path="">', paste0(' data-path="', xfun::with_ext(cur, '.html'), '">'), toc[i])
  # unnumbered sections
  r = '^<li><a href="([^#]*)(#[^"]+)">([^<]+</a>.*)$'
  i = grep(r, toc)
  toc[i] = gsub(
...
download_filenames = function(config) {
  if (length(exts <- bookdown:::load_config()[['download']]) == 0) exts = config$download
  ...
  if (is.null(book_name <- bookdown:::opts$get('book_filename'))) return()
  if (isTRUE(exts) || length(exts) == 0) {
    exts = c('pdf', 'epub', 'mobi')
    downloads = xfun::with_ext(book_name, exts)
    xfun::in_dir(bookdown:::output_path('.'), {
